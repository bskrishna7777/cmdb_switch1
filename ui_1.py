...
with col1:
    df2 = pd.read_csv('Merged_MCM_Structure.csv')
...
with col2:
    ...
    df1 = pd.read_csv('Merged_DHCP_Structure.csv')
...
if st.button("📊⬇️ Extract Only Server"):
...
# # =====================================================================================================        
# # --- SCCM Metadata Normalization Section ---
# if st.session_state.get('normalization_completed', False):
#     st.markdown("---")
#     st.header("SCCM Metadata Normalization")

#     # Load and show raw SCCM data
#     try:
#         sccm_csv_file_path = "Server_MCM_Raw.csv"
#         sccm_raw_df = pd.read_csv(sccm_csv_file_path)
#         st.subheader(f"Server SCCM Raw Data: {len(sccm_raw_df)} Records")
#         st.dataframe(sccm_raw_df, height=400)
#         st.session_state.sccm_raw_df = sccm_raw_df
#     except Exception as e:
#         st.error(f"Failed to load Server SCCM raw data: {str(e)}")
#         st.session_state.sccm_raw_df = None

#     # Button to normalize Server SCCM metadata
...
#     if st.button("🚀 Normalize SCCM Metadata", key="normalize_sccm", help="Click to start Server SCCM data transformation"):
#         run_sccm_metadata_normalization()

#     # Show SCCM metadata results if normalization is done
#     if st.session_state.get('SCCM_normalise_completed', False):
#         st.success("✅ SCCM Metadata Normalized!")
#         col1, col2 = st.columns([3, 1], vertical_alignment="center")
#         with col1:
#             st.markdown("<div style='font-size:1.8rem; font-weight:600; line-height:1.2; margin-bottom:0;'>SCCM - Normalized Metadata</div>", unsafe_allow_html=True)
#         with col2:
#             conf_str = f"{st.session_state.sccm_metadata_confidence:.2f}" if st.session_state.sccm_metadata_confidence else "N/A"
#             st.markdown(f"<div style='text-align:right; font-size:1.3rem; font-weight:600;'>Confidence Score: {conf_str}</div>", unsafe_allow_html=True)
        
#         st.dataframe(st.session_state.sccm_standardized_df, height=400)
#         csv = st.session_state.sccm_standardized_df.to_csv(index=False).encode('utf-8')
#         st.download_button(
#             label="📥 Download Metadata-Normalized SCCM",
#             data=csv,
#             file_name=f"standardized_sccm_{datetime.now().strftime('%Y%m%d')}.csv",
#             mime='text/csv'
#         )
...
#                 standardized_sccm_path = llamaindex_solution_revised.main("Server-MCM-NormMetada_dtime.xlsx", "Server-MCM-Norm2.xlsx")
...
#         with st.spinner("Combining data..."):
#             output_path = main3_server_cmdbmerge.main()
#             combined_df = pd.read_excel(output_path)
...
#         if df is not None:
#             st.subheader(f"CMDB Discovery: {len(df)}")
#             st.dataframe(df, height=400)
#             csv = df.to_csv(index=False).encode('utf-8')
#             st.download_button(
#                 label=f"📥 Download CMDB Discovered",
#                 data=csv,
#                 file_name=f"standardized_{datetime.now().strftime('%Y%m%d')}.csv",
#                 mime='text/csv',
#             )
...
#                 standardized_cmdb_df, cmdb_confidence, anomaly_count = anomalydetector.main("Semifinal CMDB.xlsx", "assets_anomalies.xlsx", "N")
...
#             for j, (output_path, name) in enumerate(zip(output_paths, output_names), 1):
#                 combined_df = pd.read_excel(output_path)
#                 st.session_state[f'normalized_d2{j}'] = combined_df
#                 st.session_state[f'normalized_df2{j}_name'] = name
...
#     for i in range(1, 3):
#         df2 = st.session_state.get(f'normalized_df2{j}')
#         name2 = st.session_state.get(f'normalized_df2{j}_name')
#         if df is not None:
#             st.subheader(f"{name2}: {len(df2)}")
#             st.dataframe(df2, height=400)
#             csv = df.to_csv(index=False).encode('utf-8')
#             st.download_button(
#                 label=f"📥 Download {name2}",
#                 data=csv,
#                 file_name=f"{name2.replace(' ', '_').lower()}_{datetime.now().strftime('%Y%m%d')}.csv",
#                 mime='text/csv'
#             )
