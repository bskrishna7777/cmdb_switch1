...
def standardize_with_llamaindex(csv_path, output_path):
    ...
    vendor_ids = df['Vendor Class ID'].dropna().tolist()
    ...
    standardize_prompt = PromptTemplate(
        ...
        The output should:
        - Clearly identify the operating system or vendor type.
    ...
    df['Standardized Vendor Class ID'] = df['Vendor Class ID'].map(lambda x: mapping.get(x, {}).get("standardized", x))
    df['Confidence Score'] = df.apply(lambda row: mapping.get(row['Vendor Class ID'], {}).get("confidence", "") if row['Standardized Vendor Class ID'] else "", axis=1)
...
def check_coverage(original_df, mapping):
    missing = []
    for vendor_id in original_df['Vendor Class ID'].dropna().unique():
        if vendor_id and vendor_id not in mapping:
            missing.append(vendor_id)
    
    coverage = 100 - (len(missing) / len(original_df['Vendor Class ID'].dropna().unique()) * 100)
...
# Example usage
def main():
    df, mapping, average_confidence = standardize_with_llamaindex("dhcp_discovery_data_1000recordsV3_modified_20250422_144321.csv", "dhcp_discovery_data_Standardized_averageconfidence.csv")
    missing = check_coverage(df, mapping)
    
    # Display sample results
    print("\nSample standardizations:")
    sample = df[['Vendor Class ID', 'Standardized Vendor Class ID', 'Confidence Score']].dropna().sample(min(10, len(df)))
    print(sample)
