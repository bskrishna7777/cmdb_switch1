...
if st.session_state.main_active_tab == 0:
    ...
    if st.session_state["server_checked"]:
        ...
        with tab_grn:
            ...
            grn_manufacturers = {}
            # Store the original processing order
            ...
            def parse_pdf(pdf_path, filename):
                ...
                po_number, grn_number = extract_po_and_grn(lines)
               
                # Link filename to GRN number for later reference
                if grn_number:
                    grn_to_filename[grn_number] = filename
                    # Add to processing order list
                    if grn_number not in file_processing_order:
                        file_processing_order.append(grn_number)
                ...
                line_index = 0  # Track the original line index for sorting
                ...
                while i < len(data_lines):
                    if data_lines[i].isdigit():
                        ...
                        j = k + 2
                        line_index_in_file = j  # Track position in the file
                        while j < len(data_lines) and not data_lines[j].isdigit():
                            for sn in serial_re.findall(data_lines[j]):
                                record = {
                                    ...
                                    "OriginalFile": filename  # Store filename for reference
                                }
            ...
            def generate_html_table(df):
                ...
                <table border="1" style="border-collapse: collapse; width: 100%;">
                    <thead><tr>
                """
                # Create headers but exclude internal sorting columns
                display_cols = [col for col in df.columns if col not in ['LineIndex', 'FileOrder', 'OriginalFile']]
                ...
                for _, row in df.iterrows():
                    ...
                    # Clean GRN number for display - remove any quotes or brackets
                    clean_grn = str(grn).strip("\"'[]() ")
                   
                    tooltip = f'''
                    <div class="tooltip">{clean_grn}
            ...
            if st.spinner("Processing..."):
                ...
                if all_records:
                    ...
                    # Add a FileOrder column based on the original processing order
                    file_order_lookup = {grn: idx for idx, grn in enumerate(file_processing_order)}
                    df["FileOrder"] = df["GRN Number"].map(file_order_lookup)
                   
                    # Sort the dataframe first by FileOrder (to preserve PDF order),
                    # then by GRN Number, and finally by LineIndex (for order within each PDF)
                    df = df.sort_values(["FileOrder", "GRN Number", "LineIndex"])
                   
                    # Reorder columns to put confidence score at the end (after GRN Number)
                    display_cols = [col for col in df.columns if col not in ["LineIndex", "FileOrder", "OriginalFile"]]
                    display_cols.remove("Confidence Score")
                    new_cols = display_cols + ["Confidence Score"]
                   
                    # Save to Excel with only display columns
                    df[new_cols].to_excel(OUTPUT_EXCEL, index=False)
